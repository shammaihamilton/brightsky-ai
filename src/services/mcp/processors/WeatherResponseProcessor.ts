...
export class WeatherResponseProcessor implements ResponseProcessor {
  ...
  async processResult(
    ...
    try {
      ...
      console.error("[WeatherResponseProcessor] Error processing weather result:", error);
      return this.createFallbackResponse(weather);
    }
  }

  private buildWeatherPrompt(weather: WeatherResult, originalMessage: string): string {
...
- Respond as a helpful weather assistant, giving a natural, conversational summary of the weather.`;
  ...
  private isAIRefusingData(response: string): boolean {
    return (
      ...
      /I don't have real-time capabilities|I can't provide current weather|I do not have access to real-time weather|I am unable to provide current weather/i.test(response)
    );
  }
