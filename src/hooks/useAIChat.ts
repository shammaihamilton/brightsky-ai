...
import { addMessageOptimistic, setConnectionStatus } from '../store/slices/chatSlice';
...
export const useAIChat = (): UseAIChatReturn => {
  ...
  const sendMessage = useCallback(async (message: string) => {
    ...
    try {
      ...
      dispatch(addMessageOptimistic({ text: message.trim() }));
      
      // Update connection status to indicate sending
      dispatch(setConnectionStatus('connecting'));
      
      // Send message to AI service
      const result = await dispatch(sendMessageToServer({ userMessage: message.trim() }));
